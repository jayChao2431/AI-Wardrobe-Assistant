...
def organize_by_category():
    ...
    for split_name, (img_file, cate_file) in splits.items():
        ...
        if len(img_paths) != len(categories):
            print(
                f"Warning: Mismatch in {split_name} - images: {len(img_paths)}, categories: {len(categories)}")
            continue
    ...
    for category in sorted(stats.keys()):
        ...
        print(
            f"{category:<25} {train_count:<10} {val_count:<10} {test_count:<10} {total:<10}")
...
def create_subset(num_categories=5, samples_per_category=200):
    ...
    top_categories = sorted(category_counts.items(),
                            key=lambda x: x[1], reverse=True)[:num_categories]
    ...
    for split in ["train", "val", "test"]:
        ...
        if split == "train":
            ...
            n_samples = samples_per_category - \
                int(samples_per_category * 0.7) - \
                int(samples_per_category * 0.15)
...
def main():
    ...
    if args.all:
        ...
        if organize_by_category():
            create_subset(num_categories=args.categories,
